...
export class GCSOutputHandler implements OutputHandler {
  ...
  setWorkspaceDetails(
    ...
    sshKeyPath?: string,
    configFiles?: string[],
    externalTestRepo?: { url: string; branch: string; installationAccessToken: string; existingPrBranch?: string; updateSubmoduleToLatest?: boolean }
  ): void {
    this.workspaceRoot = workspaceRoot;
    this.sshKeyPath = sshKeyPath;
    this.configFiles = configFiles;
    this.externalTestRepo = externalTestRepo;
    ...
    if (externalTestRepo) {
      logger.debug(`[TASK ${this.taskId}] External test repo configured: ${externalTestRepo.url}`);
    }
  }
  ...
  async onComplete(result: ClaudeRunResult, durationMs: number): Promise<void> {
    ...
    if (result.exitCode === 0 && this.postExecutionActions && this.workspaceRoot) {
      ...
      // External test repo submodule operations (BYOT)
      // Must run BEFORE parent repo git operations so the submodule pointer can be detected
      // by getChangedFiles() if updateSubmoduleToLatest was used
      if (this.externalTestRepo) {
        try {
          const gitService = new GitService();
          const submodulePath = 'tests';
          const { existingPrBranch, updateSubmoduleToLatest } = this.externalTestRepo;

          // For updateSubmoduleToLatest (merge events), the submodule HEAD was already
          // advanced by initSubmodule(). The parent git commit flow will detect the
          // submodule change via getChangedFiles() and include 'tests' automatically.
          if (updateSubmoduleToLatest) {
            logger.info(`[TASK ${this.taskId}] Submodule pointer update mode (merge event) — parent commit will include submodule change automatically`);
          } else {
            logger.info(`[TASK ${this.taskId}] Checking for changes in external test repo submodule`);
            const hasSubmoduleChanges = await gitService.hasSubmoduleChanges(this.workspaceRoot, submodulePath);

            if (hasSubmoduleChanges) {
              const { url, branch: baseBranch, installationAccessToken } = this.externalTestRepo;
              const tokenUrl = gitService.buildTokenUrl(url, installationAccessToken);
              const { owner, repo } = gitService.parseGitHubUrl(url);

              // Use existing PR branch if iterating, otherwise generate new branch name
              const branchName = existingPrBranch || (() => {
                const taskSlug = this.metadata?.taskSlug || 'task';
                const shortId = this.taskId.substring(0, 8);
                return `bugzy/${taskSlug}-${shortId}`;
              })();

              // Commit and push submodule changes
              logger.info(`[TASK ${this.taskId}] Committing and pushing external test repo changes to ${branchName}`);
              const commitMessage = `${this.metadata?.taskName || 'Task execution'}\n\n🤖 Generated by Bugzy AI\nExecution: ${this.taskId}`;
              await gitService.commitAndPushSubmodule(
                this.workspaceRoot,
                submodulePath,
                branchName,
                commitMessage,
                tokenUrl,
                { isExistingBranch: !!existingPrBranch }
              );

              // Only create PR for new branches (not when iterating on existing PR)
              if (!existingPrBranch) {
                const shortId = this.taskId.substring(0, 8);
                const prTitle = `[Bugzy] ${this.metadata?.taskName || this.metadata?.taskSlug || 'task'} - ${shortId}`;
                const prBody = [
                  `## Summary`,
                  ``,
                  `Auto-generated by Bugzy AI task execution.`,
                  ``,
                  `**Task:** ${this.metadata?.taskName || this.metadata?.taskSlug || 'task'}`,
                  `**Execution ID:** ${this.taskId}`,
                  ``,
                  `---`,
                  `🤖 Generated with [Bugzy](https://bugzy.dev)`,
                ].join('\n');
                ...
                logger.info(`[TASK ${this.taskId}] ✓ External test repo PR created: ${pr.url}`);
              } else {
                logger.info(`[TASK ${this.taskId}] ✓ Pushed iteration to existing PR branch: ${branchName}`);
              }

              // Reset submodule HEAD back to what the parent originally recorded.
              // This prevents the parent's post-execution git commit from picking up
              // the submodule pointer change — we only want that on merge events.
              logger.info(`[TASK ${this.taskId}] Resetting submodule to parent-recorded commit to prevent premature pointer update`);
              const parentGit = simpleGit(this.workspaceRoot);
              await parentGit.submoduleUpdate(['--init', submodulePath]);
            } else {
              logger.info(`[TASK ${this.taskId}] No changes in external test repo submodule`);
            }
          }
        } catch (extRepoError: any) {
          logger.error(`[TASK ${this.taskId}] Failed to process external test repo changes:`, extRepoError.message);
          logger.error(`[TASK ${this.taskId}] External test repo error details:`, extRepoError.stack || extRepoError);
          // Continue - don't fail the whole task if external repo operations fail
        }
      }
